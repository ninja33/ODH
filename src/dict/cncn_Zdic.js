...
class cncn_Zdic {
    ...
    async findZdic(word) {
        ...
        let style = doc.documentElement.querySelector('head style').innerHTML;
        let background = style.match(/url\((.+?)\)/);
        let inlinestyle = background ? style.replace(/url\((.+?)\)/, 'url(' + convert(background[1]) + ')') : '';

        let signs = style.match(/\.zdct.+\n.+?display: none;/g);
